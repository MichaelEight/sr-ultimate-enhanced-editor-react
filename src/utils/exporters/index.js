...
export function exportScenarioFile(scenarioData, settingsData) {
    let scenarioOutput = `// SCENARIO DEFINITION - ${scenarioData.scenario?.[0] || 'Unnamed'}\n`;
    ...
    for (const ext of ['cvp', 'regionincl', 'prf']) {
        for (const filename of scenarioData[ext] || []) {
    ...
    // &&MAP section
    scenarioOutput += "&&MAP\n";
    if (scenarioData.mapx && scenarioData.mapx.length > 0) {
        const mapName = scenarioData.mapx[0].split('.')[0];
    ...
    scenarioOutput += "&&SAV\n";
    if (scenarioData.sav && scenarioData.sav.length > 0) {
        const savName = scenarioData.sav[0].split('.')[0];
